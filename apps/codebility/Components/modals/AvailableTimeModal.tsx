...
const AvailableTimeModal = () => {
  ...
  function convertTo12HourFormat(time24: string, type: "start_time" | "end_time") {
    ...
    switch (type) {
      case "start_time":
        setTime((prev) => ({ ...prev, start_time: `${hours12}:${minutesFormatted} ${period}` }))
        break
      case "end_time":
        setTime((prev) => ({ ...prev, end_time: `${hours12}:${minutesFormatted} ${period}` }))
        break
      default:
        return null
    }
  ...
  return (
    ...
    >
      <DialogContent className="flex max-w-[30rem] flex-col gap-8 overflow-y-auto bg-black-100 p-6 text-white lg:p-12">
        <DialogHeader>
        ...
        <div className="flex flex-col gap-4 p-6">
          <div className="flex items-center justify-center gap-2">
            <label htmlFor="start_time" className="md:text-md w-40 text-sm lg:text-lg">
              Starting Time
            </label>
            <input
              onChange={(e) => convertTo12HourFormat(e.target.value, "start_time")}
              className="w-40 cursor-text bg-transparent p-2 text-lg text-black-500 outline-none invert"
              type="time"
              id="start_time"
          ...
          <div className="flex h-10 items-center justify-center gap-2">
            <label htmlFor="end_time" className="md:text-md w-40 text-sm lg:text-lg">
              End Time
            </label>
            <input
              onChange={(e) => convertTo12HourFormat(e.target.value, "end_time")}
              className="w-40 cursor-text bg-transparent p-2 text-lg text-black-500 outline-none invert"
              type="time"
              id="end_time"
