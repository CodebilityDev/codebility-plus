  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={() => onClose()}>
      ...
      >
        ...
        <DialogHeader>
          <DialogTitle className="text-md mb-4 text-center text-white md:text-xl">Select your Tech Stack</DialogTitle>
        </DialogHeader>

        <div className="grid w-full grid-cols-3 gap-2">
          {techstacks?.map((stack, i) => (
            <div
              className={`hover:bg-black flex cursor-pointer rounded-md border border-darkgray p-2 text-white hover:bg-white hover:text-black-500 ${
        ...
        <DialogFooter className="mt-4 flex w-full justify-end gap-2">
          <Button onClick={() => onClose()} variant="default" className="mx-auto w-[100px] sm:mx-0 sm:w-[150px]">
            Ok
          </Button>
        </DialogFooter>
      </DialogContent>
