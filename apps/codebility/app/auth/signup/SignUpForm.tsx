...
const AuthForm = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    setIsLoading(true)
    const createdUser: any = await makeApiCallWithTimeout(signupUser(data), 5000)
    ...
    if (createdUser.rawErrors) {
      ...
      if (createdUser.rawErrors.indexOf("Email already exist") !== -1) {
        setError("email_address", { type: "custom", message: "Email already exist" })
      }
      setIsLoading(false)
      return null
    }
