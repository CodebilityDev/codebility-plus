...
const AdminCard = ({ admin, color }: index_AdminCardT) => {
  ...
  return (
    <div>
      <div
        className={`flex dark:text-white text-black-600  h-full w-full flex-col items-center justify-between gap-4 rounded-lg   `}
      >
        <Image
          unoptimized
        ...
        <div className="flex w-full flex-col gap-1 capitalize">
          <p className="md:text-md text-sm dark:text-white text-black  lg:text-lg ">
            {admin.first_name} {admin.last_name}
          </p>
          {admin.main_position ? (
            <p className="dark:text-white text-black text-sm lg:text-base">
              {admin.main_position}
            </p>
          ) : (
