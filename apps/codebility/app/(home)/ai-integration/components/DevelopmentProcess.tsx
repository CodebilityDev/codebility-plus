...
import ProcessCard from "@/app/(home)/ai-integration/components/ProcessCard"
import { developmentProcessData } from "@/app/(home)/ai-integration/components/data"
    ...
    <Section className="flex flex-col gap-14 lg:hidden">
      ...
      <ol className="relative mx-auto w-72 border-s-2 border-dashed border-white">
        {developmentProcessData.map((developmentProcess) => (
          <li key={developmentProcess.id} className="mb-10 ms-14 flex flex-col gap-4">
            <div className="absolute -start-1.5 h-3 w-3 rounded-full bg-white"></div>
            <ProcessCard id={developmentProcess.id} title={developmentProcess.title} process={developmentProcess.process} />
          </li>
        ))}
      </ol>
    </Section>
