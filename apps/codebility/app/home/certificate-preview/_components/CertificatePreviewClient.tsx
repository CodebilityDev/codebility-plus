...
export default function CertificatePreview({
  ...
  /** The signed-in codev, known on the server before the client store hydrates. */
  viewerId: string;
  /** The viewer's own totals, resolved on the server so first paint needs no
   *  client fetch. Admins switching to another user still fetch that user. */
  ...
  // The store hydrates after first paint, so the server's viewerId is the
  // fallback until it does. Without it this query is disabled on mount and the
  // seeded totals would never be used.
  const pointsUserId = activeUser?.id ?? viewerId;

  const { data: pointsData, isPending: loading } = useQuery({
    queryKey: ["certificate", "points", pointsUserId],
    enabled: Boolean(pointsUserId),
    staleTime: 60_000,
    // Seeded only for the viewer's own id: the server rendered exactly that key.
    // No initialDataUpdatedAt override here (unlike the paginated tables): this
    // is a plain per-user read with no filter dimension, so the seed is current
    // and refetching it on mount would just duplicate the server's work.
    initialData: pointsUserId === viewerId ? initialPoints : undefined,
    queryFn: async () => {
