      ...
      <TableHeader className="background-lightbox_darkbox border-top hidden text-xl lg:grid">
        <TableRow className="lg:grid lg:grid-cols-5 xl:grid-cols-6">
          {headers.map((header) => (
            <TableHead
              key={header}
              className={`table-border-light_dark w-full border-b-[1px] p-4 text-left text-xl font-light ${header === "Name" ? "lg:col-span-3 xl:col-span-4" : "lg:text-center"}`}
            >
              {header}
            </TableHead>
          ))}
      ...
      <TableBody className="grid gap-5 md:grid-cols-2 lg:grid-cols-1 lg:gap-0">
        {categories.length > 0 ? (
          categories.map((category) => (
            <TableRow
              key={category.id}
              className="background-box grid lg:grid-cols-5 xl:grid-cols-6"
            >
              <TableCell className="p-4 text-xl font-light lg:col-span-3 xl:col-span-4">
                {category.name}
              </TableCell>
              ...
              <TableCell className="p-4 text-xl font-light">
                <IconDelete
                  onClick={() =>
                    onOpen("serviceCategoriesDeleteModal", category)
                  }
