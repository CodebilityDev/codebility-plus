              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-1 gap-[clamp(0.75rem,1.5vw,1rem)]">
                  ...
                  <div className="flex min-w-0 flex-1 flex-col gap-2">
                    ...
                    {/* Meta: dates + priority.
                        Dates render in the viewer's timezone, which can differ from the
                        server's, so the server HTML and the client render may disagree. */}
                    <div className="flex flex-wrap items-center gap-3 text-[clamp(0.7rem,1.4vw,0.8rem)] text-gray-500">
                      <span suppressHydrationWarning>Created {formatDate(survey.start_date)}</span>
                      {survey.end_date && (
                        <>
                          <span className="h-1 w-1 rounded-full bg-gray-600" />
                          <span suppressHydrationWarning>Expires {formatDate(survey.end_date)}</span>
                        </>
                      )}
                      <span className="h-1 w-1 rounded-full bg-gray-600" />
