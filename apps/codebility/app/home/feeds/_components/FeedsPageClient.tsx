...
import { hasReachedDailyPostLimit } from "@/actions/feeds/post";
...
export default function FeedsPageClient({
  ...
  // React keeps the previous list on screen while the new one renders, so no
  // timer or effect is needed to debounce the query.
  const deferredQuery = useDeferredValue(searchQuery);

  useOutsideClick(filterRef, filterOpen, () => setFilterOpen(false));
        ...
        <div className="mx-auto max-w-[1400px] px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-6">
            ...
            <div className="min-w-0 flex-1">
              ...
              */}
              {/* Keyed on the query so a new search resets the pagination
                  window by remounting, instead of an effect resetting state. */}
              <Feed
                key={deferredQuery}
                isAdmin={isAdmin}
