    ...
    <section className="background-navbar sticky left-0 top-0 z-20 hidden h-screen flex-col gap-14 p-6 shadow-lg max-lg:hidden lg:flex">
      <div className="flex justify-stretch gap-4 max-lg:hidden">
        ...
        >
          <Link href="/">
            <Image
              ...
              className={`hidden h-[40px] transition-all dark:block ${
                !isToggleOpen && "w-0"
              }`}
            />
            <Image
              ...
              className={`h-[40px] transition-all dark:hidden ${
                !isToggleOpen && "w-0"
              }`}
            />
          </Link>
        ...
        <Image
          ...
          height={40}
          alt="logo"
          className={`toggle-logo-btn ${
            isToggleOpen ? "close-nav" : "open-nav mx-auto"
          }`}
        />
      </div>
        ...
        {sidebarData.map((item) => {
          ...
          return (
            <div
              ...
              } ${!isToggleOpen ? "mt-0" : "mt-5"} mr-4`}
            >
              <h4
                className={`text-gray text-sm uppercase ${
                  !isToggleOpen || !hasPermission ? "hidden" : "block"
                }`}
              >
                {item.title}
              </h4>
              <div
                className={`${
                  !isToggleOpen ? "mt-0" : "mt-3"
                } flex flex-1 flex-col gap-2 max-lg:hidden`}
              >
                {item.links.map((link) => {
