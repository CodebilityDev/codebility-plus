...
import { revalidateTechnicalLeaderboard } from "@/actions/leaderboard/actions";
...
export default function WeeklyTop({
  ...
  useEffect(() => {
    ...
    // Approving a batch of tasks emits one event per row; coalesce them so the
    // board refetches once instead of once per row.
    let pending: ReturnType<typeof setTimeout> | null = null;

    const channel = supabase
      .channel("dashboard:codev_points")
      .on(
        "postgres_changes",
