  ...
  switch (from) {
    case "snake":
      if (to === "camel") {
        ...
        transform = (text: string, increment: number) =>
          text.charAt(increment + 1).toUpperCase();
    ...
    case "camel":
      if (to === "snake") {
        ...
        transform = (text: string, increment: number) =>
          `_${text.charAt(increment).toLowerCase()}`;
      }
      break;
  }
...
function transformPropertyNameCase(
...
export async function updateBuilderProfileData(
  cardId: string,
...
export async function publishProfile(
  ...
  const { data, error: fetchingCardError } = await supabase
    .from("cards")
    .select()
