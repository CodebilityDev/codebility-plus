...
async function ensureGuildRegistered(guildId, guildName) {
    const { error: upsertError } = await supabase_bot_1.supabaseBot
        .from("guilds_discord")
        .upsert({
        id: guildId,
...
const slashCommand = new discord_js_1.SlashCommandBuilder()
    ...
    .setRequired(true)
    .addChoices({ name: "Create Reward", value: "create" }, { name: "List Rewards", value: "list" }, { name: "Remove Reward", value: "remove" }))
    ...
    .setRequired(false)
    .addChoices({ name: "Role", value: "role" }, { name: "Badge", value: "badge" }, { name: "Custom Message", value: "custom_message" }))
    .addStringOption((opt) => opt
    .setName("value")
...
const execute = async (interaction) => {
    try {
        ...
        if (!interaction.memberPermissions?.has(discord_js_1.PermissionFlagsBits.ManageGuild)) {
            await interaction.reply({
                content: "❌ You do not have permission to manage rewards. (Requires **Manage Server** permission)",
                flags: [discord_js_1.MessageFlags.Ephemeral],
            });
            return;
        }
        ...
        if (action === "create") {
            ...
            if (type === "role") {
                ...
                if (!role) {
                    await interaction.editReply({
                        content: "❌ **Invalid role ID.** Please make sure the role exists in this server.",
                    });
                    return;
                }
            }
            // Check for existing reward at this level and type
            const { data: existingReward, error: fetchError } = await supabase_bot_1.supabaseBot
            ...
            console.log(`✅ Reward created — Guild: ${guildName} (${guildId}) | Level: ${level} | Type: ${type}`);
            return;
        }
        // ===== LIST REWARDS =====
        if (action === "list") {
            await interaction.deferReply();
            const { data: rewards, error } = await supabase_bot_1.supabaseBot
                .from("level_rewards_discord")
                .select("*")
                .eq("guild_id", guildId)
            ...
            const description = Object.entries(grouped)
                ...
                const rewardLines = list
                    .map((r) => {
                    const value = r.reward_type === "role"
                        ? `<@&${r.reward_value}>`
                        : r.reward_type === "badge"
        ...
        if (action === "remove") {
            ...
            const { data: existingReward, error: checkError } = await supabase_bot_1.supabaseBot
                .from("level_rewards_discord")
                .select("*")
                .eq("guild_id", guildId)
                .eq("level", level)
            ...
            // Delete the reward
            const { error: deleteError } = await supabase_bot_1.supabaseBot
                .from("level_rewards_discord")
                .delete()
                .eq("guild_id", guildId)
                .eq("level", level)
            ...
            console.log(`✅ Reward removed — Guild: ${guildName} (${guildId}) | Level: ${level} | Type: ${type}`);
            return;
        }
