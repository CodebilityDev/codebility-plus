...
export const ProductCard = ({
  ...
  return (
    ...
    <div
      role="button"
      tabIndex={0}
      className="flex-1 min-w-[9.9375em] border border-[#C2C7D0] rounded-[1.5em]  sm:min-w-[calc(50%-20px)]"
    ...
    >
      ...
      <div className="flex items-center justify-between ml-[1em]  mb-[0.71875em]  ">
        <div className="font-semibold text-[0.75rem]">
          ₱ {Number(price.toFixed(2)).toLocaleString()}
        </div>

        <button
          className="cursor-pointer transform active:scale-95 transition-transform duration-200 disabled:opacity-50"
          onClick={e => handleAddToCart(e, _id, name)}
          disabled={isLoading}
        >
          <Image
